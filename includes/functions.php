...
function pdcgpt_api_connection_and_products() {
...
function pdcgpt_crear_descripciones() {
    // Obtener la conexión al API y los productos
    list($api_url, $api_key, $products, $log, $errors, $logs) = pdcgpt_api_connection_and_products();
...
function pdcgpt_improve_descriptions() {
    ...
    foreach ($products as $product) {
        ...
        if (empty($errors)) {
            ...
            foreach ($errors as $error) {
                ...
                echo "<li>Producto #" . htmlspecialchars($error['id'], ENT_QUOTES, 'UTF-8') . ": " . htmlspecialchars($error['title'], ENT_QUOTES, 'UTF-8') . " - ";
                if (isset($error['error'])) {
                    echo htmlspecialchars($error['error'], ENT_QUOTES, 'UTF-8');
                }
                echo "</li>";
            }
            echo "</ul>";
    ...
    if (empty($logs)) {
        ...
        echo "<ul>";
        foreach ($logs as $log) {
            echo "<li>Producto #" . htmlspecialchars($log['id'], ENT_QUOTES, 'UTF-8') . ": " . htmlspecialchars($log['title'], ENT_QUOTES, 'UTF-8') . "</li>";
            echo "<blockquote>" . htmlspecialchars($log['generated_text'], ENT_QUOTES, 'UTF-8') . "</blockquote>";
        }
        echo "</ul>";
    }
...
function pdcgpt_generar_descripciones() {
...
function pdcgpt_chatgpt_import_csv() {
...
function boton_pdcgpt_crear_descripciones() {
